...
class TorchCausalLanguageModel(TorchModel[CausalLanguageModelingInference]):
    ...
    def forward(  # type: ignore[override]
        ...
        **kwargs: Any,
        ...
        source = cast(torch.LongTensor, token_ids[:, :-1])
        target = cast(torch.LongTensor, token_ids[:, 1:])
        source_mask = cast(torch.BoolTensor, mask[:, :-1])
        target_mask = cast(torch.BoolTensor, mask[:, 1:])

        embeddings = self._embedder(source)
        encodings, _ = self._decoder(embeddings, inputs_mask=source_mask)
        ...
        output = TorchCausalLanguageModelOutput(inference=inference, loss=loss)
        return output
    ...
    def infer(  # type: ignore[override]
        ...
        *,
        ...
        inference = CausalLanguageModelingInference(
            ...
            pred_mask=mask.unsqueeze(1).detach().cpu().numpy(),
        )
